//+------------------------------------------------------------------+
//|                     EA Falha Exaustao v27.0                      |
//|  BUY-only | MinAfastamentoATR=3.5 | TP abaixo da MA20           |
//+------------------------------------------------------------------+
//
//  DIAGNOSTICO v26 (970 trades, 2025.01-2026.06):
//  - Win rate: 30.72%  |  R:R real: 1.797  |  PF: 0.797
//  - Resultado: -1,247.79 EUR
//
//  BREAKDOWN POR DIRECAO (v26):
//  - BUY:  498 trades, 33.13% win rate, ~-360 EUR
//  - SELL: 472 trades, 28.18% win rate, ~-916 EUR
//
//  CAUSAS RAIZ:
//  [A] SELL perde muito: USDJPY tem tendencia de alta estrutural
//      (JPY fraco 2025-2026). Falhas de topo contra tendencia falham.
//  [B] TP ACIMA da MA20: com MinAfastATR=2.0, o TP fica +1.14xATR
//      ALEM da MA20, exigindo que o preco atravesse a media.
//      Isso e contrario a ideia de reversao para a media.
//
//  MATEMATICA DO ALINHAMENTO TP/MA20 (BUY, setup tipico):
//  - Entrada ~ H1 + 0.08xATR (acima de L2 ~ C1)
//  - dist_entrada_MA20 ~ MinAfastATR - 0.18 (em xATR)
//  - SL_dist ~ pavio(0.3) + SL_Folga(1.0) + offset(0.18) ~ 1.48xATR
//  - TP = entrada + 2x1.48xATR = entrada + 2.96xATR
//  - MA20 = entrada + (MinAfastATR - 0.18)xATR
//
//  Com MinAfastATR=2.0: MA20 esta a +1.82xATR da entrada
//                       TP esta a +2.96xATR => TP ACIMA MA20 (+1.14xATR)
//  Com MinAfastATR=3.5: MA20 esta a +3.32xATR da entrada
//                       TP esta a +2.96xATR => TP ABAIXO MA20 (-0.36xATR)
//
//  CORRECOES v27:
//  [1] PermitirVenda = false  => BUY only, evita direcao perdedora
//  [2] MinAfastamentoATR = 3.5 => TP cai naturalmente abaixo da MA20
//      (reversao para a media verdadeira, sem exigir atravessar)
//
//+------------------------------------------------------------------+
...
input bool   PermitirVenda           = false;  // OFF: SELL perdeu -916 EUR em v26
...
input double MinAfastamentoATR       = 3.5;   // Floor 2.0. Com 3.5, TP cai ABAIXO da MA20 (reversao natural)
...
int OnInit()
{
   ...
   if(gMinAfastamentoATR != MinAfastamentoATR)
      Print("AVISO v27: MinAfastamentoATR ", MinAfastamentoATR,
            " elevado ao floor de ", gMinAfastamentoATR);
   if(gSL_Folga_ATR != SL_Folga_ATR)
      Print("AVISO v27: SL_Folga_ATR ", SL_Folga_ATR,
            " elevado ao floor de ", gSL_Folga_ATR);

   Print("v27 parametros efetivos | MinAfastATR:", gMinAfastamentoATR,
...
void OnTick()
{
   ...
   if(setupCompraAtivo && ask >= precoGatilho)
   {
      ...
      {
         trade.Buy(LoteInicial, _Symbol, ask, precoStopLoss, precoTakeProfit, "Falha de Fundo v27");
   ...
   if(setupVendaAtivo && bid <= precoGatilho)
   {
      ...
      if(maxGap <= 0 || overshoot <= maxGap)
      {
         trade.Sell(LoteInicial, _Symbol, bid, precoStopLoss, precoTakeProfit, "Falha de Topo v27");
