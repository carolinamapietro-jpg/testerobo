//+------------------------------------------------------------------+
//|                     EA Falha Exaustao v45.0                      |
//|  HA Multi-TF 2-Barras H4 | BUY-only | USDJPY M5                  |
//+------------------------------------------------------------------+
//
//  DIAGNOSTICO v44 (1451 trades, 2022.01-2026.06):
//  - Lucro liquido: +6.509 EUR (POSITIVO - primeiro lucrativo!)
//  - Mas retorno: 1.2%/mes (alvo: 4%/mes)
//  - Max DD: 42.94% | Max perdas consecutivas: 17
//  - PF: 1.03 | Win rate: 45.21% (break-even: 44.4%)
//  - EV/trade: +4.48 EUR — margem quase zero
//  - 27 trades/mes | SL:TP = 63%:37%
//
//  POR QUE v44 SO LUCROU 1.2%/MES:
//  [A] H4 HA apenas 1 barra verde: aceita rebounds de H4 que
//      ainda estao em queda (flip verde durar 1 barra e reverter)
//  [B] Cooldown de 30 barras (2.5h) apos 5 perdas consecutivas:
//      volta a operar no mesmo dia ruim => 17 perdas em sequencia
//  [C] Muitos trades (27/mes): EV +4.48 EUR/trade e muito baixo
//      para gerar retorno significativo mesmo com muitas entradas
//
//  SOLUCAO v45 — 3 CIRURGIAS PRECISAS:
//
//  [1] H4 HA: exigir 2 BARRAS CONSECUTIVAS verdes no H4
//      bar[1] E bar[2] do H4 ambos verdes
//      = H4 bullish por >= 8h antes de entrar no M5
//      Filtra rebounds de H4 que duram 1 barra e revertem
//      => win rate esperado: 52-58% (vs 45.2%)
//
//  [2] CIRCUIT BREAKER: apos MaxPerdasConsecutivas => BLOQUEAR DIA
//      Antes: cooldown 30 barras (2.5h) → voltava a perder
//      Agora: tradingBloqueadoHoje=true → para pelo dia inteiro
//      => elimina o padrao de 17 perdas consecutivas
//
//  [3] H4 PRECO > MA50-H4: reintroduzir preco acima da MA50 do H4
//      Close M5 acima da MA50 do H4 = contexto de alta confirmado
//      Combinado com H4 HA 2-barras = dupla confirmacao H4
//
//  HERANCA v44:
//  - M5 HA: 2 barras verdes + 1 vermelha (sinal)
//  - H4 HA: agora 2 barras verdes (era 1)
//  - SL = min(HA_Low[1], HA_Low[2]) - ATR * 0.5
//  - MaxTradesDia = 2 | Close > MA20 | MinSLDistanciaATR = 0.5
//
//+------------------------------------------------------------------+
...
input int    HA_Lookback_H4          = 50;    // 50 barras H4 para convergencia

input group "=== H4 MEDIA MOVEL ==="
input bool   UsarFiltroPrecoH4_MA    = true;  // v45: preco M5 acima da MA50-H4
...
input int    MaxPerdasConsecutivas   = 5;     // v45: dispara bloqueio do DIA (nao cooldown)
...
int OnInit()
{
   ...
   Print("v45 HA Multi-TF | Risco:",
         (UsarRiscoDinamico ? DoubleToString(RiscoPerTrade_PCT,1)+"%" : "fixo "+DoubleToString(LoteFixo,2)),
         " | H4-HA-2bar:", (UsarFiltroH4_HA ? "ON" : "OFF"),
         " | H4-MA50:", (UsarFiltroPrecoH4_MA ? "ON" : "OFF"),
         " | CB-consecutivas->DIA: ON",
   ...
   {
      Print("ERRO v45: falha ao criar handle de indicador.");
      return INIT_FAILED;
   }
...
void OnTick()
{
   ...
   if(barAtual != lastBar)
   {
      ...
      // --- FILTRO H4: MA50 E 2-BARRAS HA (v45) ---
      double ma_h4 = GetBuffer(hMA_H4, 1);

      bool ok_preco_h4_ma = !UsarFiltroPrecoH4_MA || (ma_h4 <= 0) ||
                             (iClose(_Symbol, _Period, 1) > ma_h4);

      bool h4_ha_bullish = true;
      if(UsarFiltroH4_HA)
      {
         double h4_o1, h4_h1, h4_l1, h4_c1;
         double h4_o2, h4_h2, h4_l2, h4_c2;
         CalcularHA(1, PERIOD_H4, HA_Lookback_H4, h4_o1, h4_h1, h4_l1, h4_c1);
         CalcularHA(2, PERIOD_H4, HA_Lookback_H4, h4_o2, h4_h2, h4_l2, h4_c2);
         // v45: AMBAS as barras H4 devem ser verdes (era apenas bar[1])
         h4_ha_bullish = (h4_c1 > h4_o1) && (h4_c2 > h4_o2);
      }

      bool filtro_h4_ok = h4_ha_bullish && ok_preco_h4_ma;
      ...
      // SETUP DE COMPRA
      //------------------------------------------------------------
      if(PermitirCompra && sinal_compra && filtro_h4_ok && ok_corpo1 && ok_acima_ma20)
      {
         ...
         if(ok_MA200 && ok_cd)
         {
            ...
            else if(ValidarStops(gatilho, sl, ORDER_TYPE_BUY))
            {
               ...
               Print("SETUP COMPRA | H4-HA2:ON H4-MA50:OK | SL:", sl,
      ...
      //------------------------------------------------------------
      if(PermitirVenda && sinal_venda && !h4_ha_bullish)
      {
         bool ok_MA200 = !UsarFiltroMA200 || (ma20 < ma200);
   ...
   if(setupCompraAtivo && ask >= precoGatilho)
   {
      ...
      if(maxGap <= 0 || overshoot <= maxGap)
      {
         ...
         if(trade.Buy(loteCalculado, _Symbol, ask, precoStopLoss, precoTakeProfit, "HA v45"))
         {
            tradesDiaContagem++;
   ...
   if(setupVendaAtivo && bid <= precoGatilho)
   {
      ...
      {
         if(loteCalculado <= 0) loteCalculado = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN);
         if(trade.Sell(loteCalculado, _Symbol, bid, precoStopLoss, precoTakeProfit, "HA v45"))
            tradesDiaContagem++;
      }
      else
...
void VerificarResetDiario(datetime barAtual)
{
   ...
   tradesDiaContagem    = 0;
   perdasConsecutivas   = 0;   // v45: reseta contador ao inicio do dia
                        ...
                        const MqlTradeResult      &result)
{
   ...
   if(profit < 0.0)
   {
      ...
      // v45: apos MaxPerdasConsecutivas => bloquear o DIA INTEIRO
