//+------------------------------------------------------------------+
//|                     EA Falha Exaustao v32.0                      |
//|  Filtro slope MA20 | DD reduzido | 3.0 lots | >4%/mes            |
//+------------------------------------------------------------------+
//
//  DIAGNOSTICO v31 (287 trades BUY, 2025.01-2026.06):
//  - Win rate: 38.33% | PF: 1.23 | Sharpe: 5.14
//  - Lucro: +8.115 EUR = 477 EUR/mes = 4.77%/mes (META ATINGIDA)
//  - PROBLEMA: Max DD = 40.11% (4.681 EUR) => inaceitavel em conta real
//  - Max loss streak: 8 trades = -2.862 EUR = 28.6% da conta
//
//  CAUSA DO DD ALTO:
//  Com 4.0 lots e win rate 38.33%, qualquer sequencia de losses ativa
//  rebaixamentos severos. Cada loss = ~203 EUR = 2.03% da conta.
//  8 losses seguidos = 28.6% DD.
//
//  ESTRATEGIA v32 — REDUCAO DE DD SEM PERDER 4%/MES:
//  [1] Filtro slope MA20 (novo):
//      BUY so quando MA20[1] > MA20[2] (MA20 esta subindo)
//      => Bloqueia entradas em tendencias estagnadas/revertendo
//      => Esperado: win rate 38% -> 41-43%, menos trades ruins
//      => Menos losses consecutivos = DD menor
//
//  [2] LoteInicial = 3.0 (reduzido de 4.0):
//      Com win rate melhorado (42%), EV/trade aumenta ~78%
//      Ex.: 12 trades/mes x 1.26 EUR x 30 = 453 EUR = 4.5%/mes
//      Max DD estimado: 40% x (3.0/4.0) = 30% (vs 40%)
//
//  PROJECAO v32:
//  Sem slope ajuda (38%, 17 trades): 17 x 0.71 x 30 = 362 EUR = 3.6%
//  Com slope (42%, 13 trades):       13 x 1.26 x 30 = 492 EUR = 4.9%
//  => Se slope nao ajudar: aumentar LoteInicial para 3.5
//
//+------------------------------------------------------------------+
...
input bool   PermitirVenda           = false;  // SELL: never profitable in USDJPY

input group "=== PARAMETROS DE OPERACAO ==="
input double LoteInicial             = 3.0;   // Reduzido de 4.0; aumentar para 3.5 se necessario
...
input group "=== FILTRO DE SLOPE DA MA20 (NOVO) ==="
input bool   UsarFiltroSlopeMA20     = true;  // BUY so quando MA20 esta subindo
                                               // Eleva win rate, reduz sequencias de loss
...
int OnInit()
{
   ...
   if(gMinAfastamentoATR != MinAfastamentoATR)
      Print("AVISO v32: MinAfastamentoATR elevado ao floor de ", gMinAfastamentoATR);
   if(gSL_Folga_ATR != SL_Folga_ATR)
      Print("AVISO v32: SL_Folga_ATR elevado ao floor de ", gSL_Folga_ATR);

   double perda_est   = 5.07 * LoteInicial / 0.1;
   double lucro_est   = 11.97 * LoteInicial / 0.1;

   Print("v32 | Lote:", LoteInicial,
   ...
   Print("v32 | MinAfastATR:", gMinAfastamentoATR,
         " SlopeMA20:", (UsarFiltroSlopeMA20 ? "ON" : "OFF"),
...
void OnTick()
{
   ...
   if(barAtual != lastBar)
   {
      ...
      double ma20_ant  = GetBuffer(hMA20,  2);  // MA20 da barra anterior
      ...
      // Filtro slope: MA20 deve estar na direcao do trade
      bool ok_slope_compra = !UsarFiltroSlopeMA20 || (ma20_ant <= 0) || (ma20 >= ma20_ant);
      bool ok_slope_venda  = !UsarFiltroSlopeMA20 || (ma20_ant <= 0) || (ma20 <= ma20_ant);
      ...
      if(PermitirVenda && C1 > ma20 && distMA20 >= distMin)
      {
         ...
         if(ok_MA200 && ok_pavio && ok_padrao_venda && ok_cd && ok_slope_venda)
         {
      ...
      if(PermitirCompra && C1 < ma20 && distMA20 >= distMin)
      {
         ...
         if(ok_MA200 && ok_pavio && ok_padrao_compra && ok_cd && ok_slope_compra)
         {
            ...
            if(ValidarStops(gatilho, sl, ORDER_TYPE_BUY))
            {
               ...
               Print("SETUP COMPRA | SL:", sl, " TP:", tp,
                     " risco:", NormalizeDouble(risco/atr,2), "xATR",
                     " MA20:", NormalizeDouble(ma20,5),
                     " slope:", NormalizeDouble(ma20-ma20_ant,5));
            }
         }
      }
   }
   ...
   if(setupCompraAtivo && ask >= precoGatilho)
   {
      ...
      {
         trade.Buy(LoteInicial, _Symbol, ask, precoStopLoss, precoTakeProfit, "Falha de Fundo v32");
   ...
   if(setupVendaAtivo && bid <= precoGatilho)
   {
      ...
      if(maxGap <= 0 || overshoot <= maxGap)
      {
         trade.Sell(LoteInicial, _Symbol, bid, precoStopLoss, precoTakeProfit, "Falha de Topo v32");
