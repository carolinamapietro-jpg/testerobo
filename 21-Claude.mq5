//+------------------------------------------------------------------+
//|                     EA Falha Exaustao v21.0                      |
//|  Filtro de slope MA20 + protecao anti-gap + floor 2.0            |
//+------------------------------------------------------------------+
//
//  DIAGNOSTICO v20 (dados reais):
//
//  SELL:  151 trades | 35.10% win rate | perde ~116 EUR/ano
//  BUY:   206 trades | 43.69% win rate | ganha  ~37 EUR/ano
//  NET:   357 trades | 40.06% win rate | -77.18 EUR/ano
//  Break-even necessario: 42.05% (avg_win=6.38 / avg_loss=4.63)
//  Gap atual: -1.99 pp abaixo do break-even
//
//  CAUSA DO SELL RUIM:
//  O filtro MA20<MA200 permite SELL em correcoes temporarias de baixa
//  no USDJPY, mas o USDJPY teve tendencia de alta forte em 2025-2026.
//  O MA20 fica brevemente abaixo do MA200 sem momentum real de baixa.
//  Resultado: SELL entra contra a tendencia dominante => 35% win rate.
//
//  ALERTA: operacao minima de 7 segundos detectada no v20.
//  Causa: entrada executada com preco ja muito distante do gatilho
//  (gap de abertura de barra). SL e atingido imediatamente.
//
//  CORRECOES v21:
//  [1] Filtro de slope MA20 (5 barras):
//      SELL so permitido se MA20[1] < MA20[6] (MA20 caindo ativamente)
//      BUY  so permitido se MA20[1] > MA20[6] (MA20 subindo ativamente)
//      Elimina entradas onde MA20 esta abaixo/acima de MA200 mas sem
//      momentum direcional real. Principal correcao do SELL ruim.
//  [2] Protecao anti-gap (0.5 x ATR):
//      Entrada cancelada se preco atual esta mais de 0.5xATR alem do
//      gatilho no momento do disparo. Previne as operacoes de 7s.
//      Setup cancelado; aguarda proximo sinal valido.
//  [3] Floor de MinAfastamentoATR elevado de 1.5 para 2.0:
//      Filtra setups marginais adicionais. Com 2.0xATR minimo de
//      afastamento da MA20, apenas reversoes em preco muito estendido
//      sao consideradas (maior probabilidade de reverter a media).
//  [4] Mantidos os floors do v20: SL_Folga>=0.8, BreakEven>=1.5
//
...
input int    SlopeBarras              = 5;    // Barras para medir slope do MA20
...
double lastATR           = 0.0;  // ATR do bar de sinal, para protecao anti-gap
...
int OnInit()
{
   ...
   if(gMinAfastamentoATR != MinAfastamentoATR)
      Print("AVISO v21: MinAfastamentoATR ", MinAfastamentoATR,
            " elevado ao floor de ", gMinAfastamentoATR);
   if(gSL_Folga_ATR != SL_Folga_ATR)
      Print("AVISO v21: SL_Folga_ATR ", SL_Folga_ATR,
            " elevado ao floor de ", gSL_Folga_ATR);
   if(gBreakEvenATR != BreakEvenATR)
      Print("AVISO v21: BreakEvenATR ", BreakEvenATR,
            " elevado ao floor de ", gBreakEvenATR);

   Print("v21 parametros efetivos | MinAfastATR:", gMinAfastamentoATR,
         " SL_Folga:", gSL_Folga_ATR, " BreakEven:", gBreakEvenATR,
         " SlopeBarras:", SlopeBarras);
...
void OnTick()
{
   ...
   if(barAtual != lastBar)
   {
      ...
      // Slope do MA20: compara valor atual com N barras atras
      // Garante que a tendencia tem momentum real, nao apenas direcao
      int    slopeIdx  = 1 + SlopeBarras;
      double ma20_old  = GetBuffer(hMA20, slopeIdx);
      bool   ma20_sobe = (ma20_old > 0 && ma20 > ma20_old);
      bool   ma20_cai  = (ma20_old > 0 && ma20 < ma20_old);
      ...
      lastATR = atr;  // persiste para protecao anti-gap no BLOCO 2

      // Padroes primarios (identicos ao v13/v19/v20)
      bool sell_cond = (H1 > H2 && C1 <= H2);
      bool buy_cond  = (L1 < L2 && C1 >= L2);

      // Desambiguacao para outside bars apenas
      bool ok_padrao_venda  = sell_cond && (!buy_cond  || C1 < O1);
      bool ok_padrao_compra = buy_cond  && (!sell_cond || C1 > O1);

      //------------------------------------------------------------
      // SETUP DE VENDA - Falha de Topo
      // Exige: MA20<MA200 (downtrend macro) E MA20 caindo (momentum)
      //------------------------------------------------------------
      if(PermitirVenda && C1 > ma20 && distMA20 >= distMin)
      {
         ...
         bool ok_slope  = ma20_cai;  // MA20 precisa estar caindo ativamente
         bool ok_limite = (tradesVendaHoje  < MaxTradesPorDirecao);
         bool ok_cd     = (cooldownVenda    == 0);
         bool ok_pavio  = (pavioSup         >= pavioMin);

         if(ok_MA200 && ok_slope && ok_limite && ok_cd && ok_pavio && ok_padrao_venda)
         {
            ...
            if(ValidarStops(gatilho, sl, ORDER_TYPE_SELL))
            {
               ...
               Print("SETUP VENDA | L1:", L1, " H1:", H1, " H2:", H2,
                     " SL:", sl, " TP:", tp,
                     " slope:", DoubleToString((ma20-ma20_old)/atr, 3), "xATR");
            }
         }
      }
      ...
      // Exige: MA20>MA200 (uptrend macro) E MA20 subindo (momentum)
      //------------------------------------------------------------
      if(PermitirCompra && C1 < ma20 && distMA20 >= distMin)
      {
         ...
         bool ok_MA200  = !UsarFiltroMA200 || (ma20 > ma200);
         bool ok_slope  = ma20_sobe;  // MA20 precisa estar subindo ativamente
         bool ok_limite = (tradesCompraHoje < MaxTradesPorDirecao);
         bool ok_cd     = (cooldownCompra   == 0);
         bool ok_pavio  = (pavioInf         >= pavioMin);

         if(ok_MA200 && ok_slope && ok_limite && ok_cd && ok_pavio && ok_padrao_compra)
         {
   ...
   //
   // Protecao anti-gap: se o preco esta mais de 0.5xATR alem do
   // gatilho no momento do disparo, a entrada e cancelada.
   // Previne operacoes que abrem e fecham em segundos por SL imediato.
   //================================================================
   ...
   if(setupCompraAtivo && ask >= precoGatilho)
   {
      ...
      {
         if(trade.Buy(LoteInicial, _Symbol, ask, precoStopLoss, precoTakeProfit, "Falha de Fundo v21"))
            tradesCompraHoje++;
         Print("COMPRA executada | Ask:", ask, " SL:", precoStopLoss, " TP:", precoTakeProfit,
               " overshoot:", DoubleToString(overshoot/lastATR, 3), "xATR");
      }
      else
      {
         Print("COMPRA cancelada (anti-gap): overshoot ",
               DoubleToString(overshoot/lastATR, 2), "xATR > 0.5xATR");
      }
      setupCompraAtivo = false;
   }
   ...
   if(setupVendaAtivo && bid <= precoGatilho)
   {
      ...
      if(maxGap <= 0 || overshoot <= maxGap)
      {
         if(trade.Sell(LoteInicial, _Symbol, bid, precoStopLoss, precoTakeProfit, "Falha de Topo v21"))
            tradesVendaHoje++;
      ...
      {
         Print("VENDA cancelada (anti-gap): overshoot ",
               DoubleToString(overshoot/lastATR, 2), "xATR > 0.5xATR");
      }
      setupVendaAtivo = false;
   }
