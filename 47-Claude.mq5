//+------------------------------------------------------------------+
//|                     EA Falha Exaustao v47.0                      |
//|  HA Multi-TF + H4-ADX + RSI | BUY-only | USDJPY M5              |
//+------------------------------------------------------------------+
//
//  HISTORICO DE VERSOES (2022.01-2026.06):
//  v42: -6475 | win 40.9% | payoff 1.14 | 401  trades
//  v43: -6303 | win 42.5% | payoff 1.23 | 972  trades
//  v44: +6509 | win 45.2% | payoff 1.25 | 1451 trades <- MELHOR
//  v45: -4896 | win 43.7% | payoff 1.21 | 921  trades (H4 2-barras)
//  v46: -4438 | win 43.9% | payoff 1.24 | 1288 trades (ADX M5)
//
//  ANALISE v46:
//  ADX no M5 foi CONTRAPRODUCENTE.
//  - Removeu 163 boas entradas da v44 (1451 -> 1288 trades)
//  - Win rate CAIU de 45.2% para 43.87% (pior que v44!)
//  - Break-even requer ~44.7%; ficamos em 43.9% -> EV negativo
//  - M5 ADX > 20 oscila muito: captura bursts de range como "tendencia"
//    e bloqueia entradas validas em tendencias com pausa (ADX < 20)
//  - Max DD explodiu para 77% (era ~25% na v44)
//
//  RAIZ DO PROBLEMA v46:
//  ADX no M5 e RUIDO — a janela de 14 barras de 5 minutos (1h10m)
//  nao discrimina range vs tendencia de forma confiavel.
//  O filtro H4 HA (ja existente desde v44) ja faz isso melhor.
//
//  SOLUCAO v47 — BASE v44 + H4-ADX + RSI:
//
//  [1] SUBSTITUIR M5-ADX por H4-ADX
//      ADX(14) calculado no H4 = 56h de dados = muito mais estavel
//      ADX_H4 > 20 = USDJPY em tendencia real no H4 -> permite entrada
//      ADX_H4 < 20 = range no H4 -> bloqueia (muito mais confiavel)
//      Implementacao: iADX(_Symbol, PERIOD_H4, ADX_Periodo)
//
//  [2] FILTRO RSI(14) > RSI_Min (padrao: 50) no M5
//      RSI > 50 = momentum net bullish nos ultimos 14 periodos
//      Complementa o HA flip: exige que o flip seja acompanhado
//      de momentum real (nao apenas 1-2 barras de pullback menor)
//      Elimina flips frageis em mercado sem forca direcional
//
//  [3] CIRCUIT BREAKER CONSECUTIVO -> DIA (de v45/v46)
//      MaxPerdasConsecutivas = 6 (era 5, ligeiramente menos agressivo)
//      5 perdas consecutivas pode acontecer por chance em trend valido
//      6 perdas e sinal mais claro de dia ruim
//
//  HERANCA v44 (mantida):
//  - M5 HA: 2 barras verdes + 1 vermelha (sinal de flip)
//  - H4 HA: 1 barra verde (bar[1] H4 verde = inicio da tendencia H4)
//  - SL = min(HA_Low[1], HA_Low[2]) - ATR * 0.5
//  - Close > MA20 | MA20 > MA200 | MaxTradesDia=2
//  - Partial TP em 1.5R + breakeven | Saida dinamica
//
//+------------------------------------------------------------------+
...
input group "=== ADX H4 (v47 — substituiu M5-ADX) ==="
input bool   UsarFiltroADX           = true;
input int    ADX_Periodo             = 14;
input double ADX_Min                 = 20.0; // ADX H4 abaixo disto = range = sem trade

input group "=== RSI M5 (v47 — confirmacao de momentum) ==="
input bool   UsarFiltroRSI           = true;
input int    RSI_Periodo             = 14;
input double RSI_Min                 = 50.0; // RSI abaixo disto = momentum fraco = sem trade
...
input int    MaxPerdasConsecutivas   = 6;   // era 5 em v46; 6 = menos agressivo
...
int OnInit()
{
   ...
   Print("v47 HA+H4ADX+RSI | Risco:",
         (UsarRiscoDinamico ? DoubleToString(RiscoPerTrade_PCT,1)+"%" : "fixo "+DoubleToString(LoteFixo,2)),
         " | H4-HA:", (UsarFiltroH4_HA ? "ON" : "OFF"),
         " | H4-ADX(", ADX_Periodo, ")>", ADX_Min, ":", (UsarFiltroADX ? "ON" : "OFF"),
         " | RSI(", RSI_Periodo, ")>", RSI_Min, ":", (UsarFiltroRSI ? "ON" : "OFF"),
         " | MaxTradesDia:", MaxTradesDia,
         " | CB-consec(", MaxPerdasConsecutivas, ")->Dia: ON");
   ...
   hADX       = iADX (_Symbol, PERIOD_H4,  ADX_Periodo);   // v47: H4 em vez de M5
   ...
   {
      Print("ERRO v47: falha ao criar handle de indicador.");
      return INIT_FAILED;
   }
...
//+------------------------------------------------------------------+
// Calcula Heikin Ashi para bar_idx no timeframe tf com 'lookback'
// barras de aquecimento para convergencia do HA_Open.
//+------------------------------------------------------------------+
...
void OnTick()
{
   ...
   if(barAtual != lastBar)
   {
      ...
      double adx_val   = GetBuffer(hADX,       1); // v47: buffer 0 do ADX no H4
      double rsi_val   = GetBuffer(hRSI,       1); // v47: RSI M5 bar[1]

      if(ma20 <= 0 || atr <= 0) return;
      if(UsarFiltroMA200 && ma200 <= 0) return;

      // --- FILTRO H4-ADX (v47: bloqueia range no H4) ---
      if(UsarFiltroADX && adx_val > 0 && adx_val < ADX_Min)
      {
         return; // H4 em range, nao operar
      }

      // --- FILTRO RSI M5 (v47: confirma momentum bullish) ---
      if(UsarFiltroRSI && rsi_val > 0 && rsi_val < RSI_Min)
      {
         return; // momentum fraco/bearish, nao operar em compra
      }
      ...
      // --- FILTRO H4 HA (1 barra — v44 approach) ---
   ...
   if(setupCompraAtivo && ask >= precoGatilho)
   {
      ...
      if(maxGap <= 0 || overshoot <= maxGap)
      {
         ...
         if(trade.Buy(loteCalculado, _Symbol, ask, precoStopLoss, precoTakeProfit, "HA v47"))
         {
            tradesDiaContagem++;
   ...
   if(setupVendaAtivo && bid <= precoGatilho)
   {
      ...
      {
         if(loteCalculado <= 0) loteCalculado = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN);
         if(trade.Sell(loteCalculado, _Symbol, bid, precoStopLoss, precoTakeProfit, "HA v47"))
            tradesDiaContagem++;
      }
      else
...
void VerificarResetDiario(datetime barAtual)
{
   ...
   tradesDiaContagem    = 0;
   perdasConsecutivas   = 0;  // reset a cada novo dia
                        ...
                        const MqlTradeResult      &result)
{
   ...
   if(profit < 0.0)
   {
      ...
      // v47: apos MaxPerdasConsecutivas => bloquear DIA INTEIRO
