//+------------------------------------------------------------------+
//|                     EA Falha Exaustao v30.0                      |
//|  Calibrado para 4%/mes | 2.0 lots | +BUY+SELL | cooldown 3      |
//+------------------------------------------------------------------+
//
//  DIAGNOSTICO v29 (184 trades, 2025.01-2026.06):
//  - BUY-only | Win rate: 39.13% | PF: 1.32 | Sharpe: 8.54
//  - Max DD: 0.80% (excelente!) | Lucro: +186.77 EUR em 17 meses
//  - PROBLEMA: apenas 10.8 trades/mes => 11 EUR/mes @ 0.1 lot
//
//  CALCULO PARA 4%/MES = 400 EUR/MES (conta 10.000 EUR):
//  - Com 10.8 trades/mes: precisa 400/(11x10) = 3.64 lots (risco alto)
//  - Com mais sinais (22 trades/mes): precisa 400/(22x10) = 1.8 lots
//
//  ESTRATEGIA v30 para atingir 4%/mes:
//  [1] MinAfastamentoATR = 1.7 (floor: 1.5)
//      Com SL_Folga=0.2: TP fica -0.04xATR da MA20 (no alvo de reversao)
//      Mais sinais que 2.0, mesma qualidade de alinhamento TP/MA20
//  [2] PermitirVenda = true (MA200 controla quando SELL e valido)
//      Adiciona 5-10 trades SELL/mes em periodos de downtrend
//  [3] CooldownBarrasSL = 3 (reduzido de 6 para mais entradas)
//  [4] LoteInicial = 2.0 (calibrado: 22 trades/mes x 1.01 EUR x 20 = 444 EUR/mes)
//
//  RISCO @ 2.0 lots em conta 10K:
//  - Perda media por trade: ~105 EUR = 1.05% da conta
//  - Max 10 losses consecutivas: ~1.050 EUR = 10.5% DD
//  - Max DD historico (v29 escalado): 0.80% x 20 = 16% (cenario extremo)
//
//+------------------------------------------------------------------+
...
input bool   PermitirVenda           = true;   // MA200 controla: SELL so em downtrend

input group "=== PARAMETROS DE OPERACAO ==="
input double LoteInicial             = 2.0;   // Calibrado para ~4%/mes em conta 10K EUR
input double RiscoRetorno            = 2.0;

input group "=== THRESHOLDS EM ATR ==="
input double MinAfastamentoATR       = 1.7;   // Floor: 1.5. Com SL_Folga=0.2, TP fica no MA20
input double PavioMinimoATR          = 0.25;  // Pavio minimo em ATR
input double SL_Folga_ATR            = 0.2;   // Floor: 0.1 — SL colado ao pavio; TP abaixo MA20
...
input int    CooldownBarrasSL        = 3;     // 3 barras = 15min no M5 (reduzido de 6)
...
double gSL_Folga_ATR;       // floor: 0.1
...
int OnInit()
{
   ...
   gMinAfastamentoATR = MathMax(MinAfastamentoATR, 1.5);  // floor reduzido
   gSL_Folga_ATR      = MathMax(SL_Folga_ATR,      0.1);

   if(gMinAfastamentoATR != MinAfastamentoATR)
      Print("AVISO v30: MinAfastamentoATR elevado ao floor de ", gMinAfastamentoATR);
   if(gSL_Folga_ATR != SL_Folga_ATR)
      Print("AVISO v30: SL_Folga_ATR elevado ao floor de ", gSL_Folga_ATR);

   // Diagnostico de alinhamento TP/MA20
   double risco_est       = 0.5 + gSL_Folga_ATR + 0.08;  // estimativa em xATR
   double tp_vs_ma20      = gMinAfastamentoATR - 0.18 - 2.0 * risco_est;
   string alinhamento     = (tp_vs_ma20 <= 0) ? "ABAIXO/NO MA20 [bom]" : "ACIMA MA20 [ruim]";

   double perda_media_estimada = 5.22 * LoteInicial / 0.1;  // escala do v29

   Print("v30 init | MinAfastATR:", gMinAfastamentoATR,
         " SL_Folga:", gSL_Folga_ATR,
         " TP estimado: ", NormalizeDouble(tp_vs_ma20,2), "xATR ", alinhamento);
   Print("v30 risco | Lote:", LoteInicial,
         " | Perda media/trade estimada: ", NormalizeDouble(perda_media_estimada,2),
         " EUR | Max 10 losses: ", NormalizeDouble(perda_media_estimada*10,2), " EUR");
...
void OnTick()
{
   ...
   if(setupCompraAtivo && ask >= precoGatilho)
   {
      ...
      {
         trade.Buy(LoteInicial, _Symbol, ask, precoStopLoss, precoTakeProfit, "Falha de Fundo v30");
   ...
   if(setupVendaAtivo && bid <= precoGatilho)
   {
      ...
      if(maxGap <= 0 || overshoot <= maxGap)
      {
         trade.Sell(LoteInicial, _Symbol, bid, precoStopLoss, precoTakeProfit, "Falha de Topo v30");
