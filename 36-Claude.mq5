//+------------------------------------------------------------------+
//|                     EA Falha Exaustao v36.0                      |
//|  Filtro tendencia H4 + SELL ativado | 3.5 lots | USDJPY M5       |
//+------------------------------------------------------------------+
//
//  DIAGNOSTICO v35 out-of-sample 2023-2024 (303 trades):
//  - Win rate: 33.99% | PF: 0.85 | Sharpe: -5.00
//  - Lucro: -6.545 EUR | Max DD: 71.96%
//  - Max 18 losses consecutivos = -6.326 EUR
//  - Filtro ATR(14)/ATR(100) > 1.8 removeu apenas 10 trades (313->303)
...
int OnInit()
{
   ...
   if(gMinAfastamentoATR != MinAfastamentoATR)
      Print("AVISO v36: MinAfastamentoATR elevado ao floor de ", gMinAfastamentoATR);
   if(gSL_Folga_ATR != SL_Folga_ATR)
      Print("AVISO v36: SL_Folga_ATR elevado ao floor de ", gSL_Folga_ATR);

   Print("v36 | Lote:", LoteInicial,
   ...
   {
      Print("ERRO v36: falha ao criar handle de indicador.");
      return INIT_FAILED;
   }
...
void OnTick()
{
   ...
   if(setupCompraAtivo && ask >= precoGatilho)
   {
      ...
      {
         trade.Buy(LoteInicial, _Symbol, ask, precoStopLoss, precoTakeProfit, "Falha de Fundo v36");
   ...
   if(setupVendaAtivo && bid <= precoGatilho)
   {
      ...
      if(maxGap <= 0 || overshoot <= maxGap)
      {
         trade.Sell(LoteInicial, _Symbol, bid, precoStopLoss, precoTakeProfit, "Falha de Topo v36");
