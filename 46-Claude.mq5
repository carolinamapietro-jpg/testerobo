//+------------------------------------------------------------------+
//|                     EA Falha Exaustao v46.0                      |
//|  HA Multi-TF + ADX Tendencia | BUY-only | USDJPY M5              |
//+------------------------------------------------------------------+
//
...
//  v45: -4896 | win 43.7% | payoff 1.21 | 921  trades ← PIOR (filtro demais)
//
//  ANALISE v45:
//  H4 HA 2-barras + H4 MA50 = PIOR que v44!
//  Por que? H4 HA 1-barra captura o INICIO das tendencias H4.
//  Exigir 2 barras H4 = entra tarde (movimento ja aconteceu).
//  H4 MA50 acrescenta lag adicional — idem.
//  Resultado: win rate caiu 45.2% → 43.7%, voltou ao prejuizo.
//
//  RAIZ DO PROBLEMA (64% SL em todas versoes):
//  Os 64% de stops vem de MERCADO EM RANGE (ADX < 20).
//  Quando USDJPY esta lateral (Jan-Fev 2022, varias janelas 2023-24),
//  o HA flip no M5 e puro ruido — flip verde → flip vermelho → flip verde
//  sem direcao real. Nenhum filtro de MA ou H4 HA elimina isso.
//  ADX < 20 = mercado lateral = BLOQUEAR TODAS as entradas.
//
//  SOLUCAO v46 — BASE v44 + ADX:
//
//  [1] VOLTA AO v44: H4 HA 1-barra verde (nao 2 barras)
//      Captura entradas no INICIO das tendencias H4 (melhor qualidade)
//
//  [2] FILTRO ADX (14) > ADX_Min (padrao: 20)
//      ADX > 20 = mercado em tendencia real → permite entradas
//      ADX < 20 = mercado lateral/range → BLOQUEIA todas entradas
//      Elimina o principal gerador de false signals no M5 HA
//
//  [3] CIRCUIT BREAKER CONSECUTIVO → BLOQUEAR DIA (de v45)
//      5 perdas consecutivas → tradingBloqueadoHoje = true
//      Nao voltamos a operar no mesmo dia ruim
//      perdasConsecutivas reset no inicio de cada dia
//
//  SEM H4 MA50 (remover — lag excessivo, prejudicou v45)
//
//  HERANCA v44:
//  - M5 HA: 2 barras verdes + 1 vermelha
//  - H4 HA: 1 barra verde (bar[1] H4 verde)
//  - SL = min(HA_Low[1], HA_Low[2]) - ATR * 0.5
//  - Close > MA20 | MaxTradesDia=2 | MinSLDistanciaATR=0.5
//
//+------------------------------------------------------------------+
...
input group "=== ADX (v46 — filtro de tendencia M5) ==="
input bool   UsarFiltroADX           = true;
input int    ADX_Periodo             = 14;
input double ADX_Min                 = 20.0; // ADX abaixo disto = range = sem trade
...
input int    MaxPerdasConsecutivas   = 5;   // dispara bloqueio do DIA
...
//  HANDLES
//==================================================================
int hMA200, hMA20, hATR, hATR_Longo, hADX;
...
void OnTick()
{
   ...
   if(barAtual != lastBar)
   {
      ...
      double adx_val   = GetBuffer(hADX,       1); // buffer 0 = linha ADX

      if(ma20 <= 0 || atr <= 0) return;
      if(UsarFiltroMA200 && ma200 <= 0) return;

      // --- FILTRO ADX (v46: bloqueia mercado em range) ---
      if(UsarFiltroADX && adx_val > 0 && adx_val < ADX_Min)
      {
         return; // mercado lateral, nao operar
      }
      ...
      if(PermitirCompra && sinal_compra && h4_ha_bullish && ok_corpo1 && ok_acima_ma20)
      {
         ...
         if(ok_MA200 && ok_cd)
         {
            ...
            else if(ValidarStops(gatilho, sl, ORDER_TYPE_BUY))
            {
               ...
               Print("SETUP COMPRA | ADX:", NormalizeDouble(adx_val,1),
   ...
   if(setupCompraAtivo && ask >= precoGatilho)
   {
      ...
      if(maxGap <= 0 || overshoot <= maxGap)
      {
         ...
         if(trade.Buy(loteCalculado, _Symbol, ask, precoStopLoss, precoTakeProfit, "HA v46"))
         {
            tradesDiaContagem++;
            Print("COMPRA exec | ADX:", NormalizeDouble(GetBuffer(hADX,0),1),
   ...
   if(setupVendaAtivo && bid <= precoGatilho)
   {
      ...
      {
         if(loteCalculado <= 0) loteCalculado = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN);
         if(trade.Sell(loteCalculado, _Symbol, bid, precoStopLoss, precoTakeProfit, "HA v46"))
            tradesDiaContagem++;
      }
      else
                        ...
                        const MqlTradeResult      &result)
{
   ...
   if(profit < 0.0)
   {
      ...
      // v46: apos MaxPerdasConsecutivas => bloquear DIA INTEIRO
