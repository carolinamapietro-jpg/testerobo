//+------------------------------------------------------------------+
//|                     EA Falha Exaustao v25.0                      |
//|  Remove limite de trades diarios por direcao                     |
//+------------------------------------------------------------------+
//
//  CORRECAO v25:
//  Removido MaxTradesPorDirecao: sem limite de operacoes por dia.
//  O EA executa todos os setups validos que encontrar.
//
//  MANTIDOS DO v24:
//  PermitirVenda=true, UsarFiltroMA200=false, SaidaDinamicaMinRR=2.0,
//  floors 2.0/0.8/1.5, anti-gap 0.5xATR, CooldownBarrasSL=12
//
//  MATEMATICA BASE (v23, 769 trades):
//  Taxa acerto 36.93% > break-even 33.33% (RR 2:1)
//  FatorLucro esperado v24/v25: ~1.17
//
//+------------------------------------------------------------------+
...
input int    CooldownBarrasSL        = 12;    // Barras M5 bloqueadas apos SL (60 min)
...
int OnInit()
{
   ...
   if(gMinAfastamentoATR != MinAfastamentoATR)
      Print("AVISO v25: MinAfastamentoATR ", MinAfastamentoATR,
            " elevado ao floor de ", gMinAfastamentoATR);
   if(gSL_Folga_ATR != SL_Folga_ATR)
      Print("AVISO v25: SL_Folga_ATR ", SL_Folga_ATR,
            " elevado ao floor de ", gSL_Folga_ATR);
   if(gBreakEvenATR != BreakEvenATR)
      Print("AVISO v25: BreakEvenATR ", BreakEvenATR,
            " elevado ao floor de ", gBreakEvenATR);

   Print("v25 parametros efetivos | MinAfastATR:", gMinAfastamentoATR,
...
void OnTick()
{
   ...
   if(barAtual != lastBar)
   {
      ...
      if(PermitirVenda && C1 > ma20 && distMA20 >= distMin)
      {
         ...
         if(ok_MA200 && ok_cd && ok_pavio && ok_padrao_venda)
         {
      ...
      if(PermitirCompra && C1 < ma20 && distMA20 >= distMin)
      {
         ...
         if(ok_MA200 && ok_cd && ok_pavio && ok_padrao_compra)
         {
   ...
   if(setupCompraAtivo && ask >= precoGatilho)
   {
      ...
      {
         trade.Buy(LoteInicial, _Symbol, ask, precoStopLoss, precoTakeProfit, "Falha de Fundo v25");
   ...
   if(setupVendaAtivo && bid <= precoGatilho)
   {
      ...
      if(maxGap <= 0 || overshoot <= maxGap)
      {
         trade.Sell(LoteInicial, _Symbol, bid, precoStopLoss, precoTakeProfit, "Falha de Topo v25");
