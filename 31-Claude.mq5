//+------------------------------------------------------------------+
//|                     EA Falha Exaustao v31.0                      |
//|  BUY-only DEFINITIVO | 4.0 lots | >4%/mes em USDJPY M5          |
//+------------------------------------------------------------------+
//
//  DIAGNOSTICO v30 (534 trades, 2025.01-2026.06):
//  - BUY: 287 trades, 38.33% win rate => LUCRATIVO
//  - SELL: 247 trades, 25.91% win rate => -5.800 EUR (destruiu tudo)
//  - PF: 0.94 | Max DD: 50.83% | Net: -2.006 EUR
//
//  CONCLUSAO DEFINITIVA SOBRE SELL EM USDJPY 2025-2026:
//  Todas as versoes testadas (v26 ao v30), com ou sem filtro MA200,
//  com SL frouxo ou colado ao pavio, o SELL nunca passou de 30.1%
//  win rate. USDJPY em tendencia estrutural de alta (JPY fraco):
//  periodos de MA20<MA200 sao correcoes no bull market, nao
//  downtrends verdadeiros. SELL desabilitado permanentemente.
//
//  CALCULO PARA 4%/MES (dados reais do v30 BUY, 0.1 lot):
//  - 287 trades / 17 meses = 16.9 trades/mes
//  - EV por trade (0.1 lot) = 0.3833 x 9.80 - 0.6167 x 5.01 = 0.67 EUR
//  - Lucro mensal @ 0.1 lot = 16.9 x 0.67 = 11.3 EUR/mes
//  - Para 400 EUR/mes (4% em 10K): 400 / 11.3 / 10 = 3.54 lots
//  - LoteInicial = 4.0 para margem acima de 4% (452 EUR/mes = 4.52%)
//
//  RISCO @ 4.0 lots em 10K EUR:
//  - Perda media/trade: 5.01 EUR x 40 = ~200 EUR = 2.0% da conta
//  - 3 losses consecutivos (media): 600 EUR = 6.0% DD (normal)
//  - 10 losses consecutivos (max historico v30 BUY): ~2.000 EUR = 20%
//  - Para reduzir risco: use LoteInicial=2.5 (alvo 2.8%/mes)
//
//+------------------------------------------------------------------+
...
input bool   PermitirVenda           = false;  // SELL: 25.91% win em v30 — desabilitado DEFINITIVAMENTE

input group "=== PARAMETROS DE OPERACAO ==="
input double LoteInicial             = 4.0;   // 4.0 lots = ~452 EUR/mes = 4.52% em 10K
                                               // Para menor risco: 2.5 lots = ~2.8%/mes
input double RiscoRetorno            = 2.0;

input group "=== THRESHOLDS EM ATR ==="
input double MinAfastamentoATR       = 1.7;   // Floor: 1.5 | gera ~17 trades BUY/mes
input double PavioMinimoATR          = 0.25;  // Pavio minimo
input double SL_Folga_ATR            = 0.2;   // Floor: 0.1 | SL colado ao pavio; TP no MA20
...
input bool   UsarFiltroMA200          = true;  // BUY: MA20 > MA200 (uptrend confirmado)
...
int OnInit()
{
   ...
   if(gMinAfastamentoATR != MinAfastamentoATR)
      Print("AVISO v31: MinAfastamentoATR elevado ao floor de ", gMinAfastamentoATR);
   if(gSL_Folga_ATR != SL_Folga_ATR)
      Print("AVISO v31: SL_Folga_ATR elevado ao floor de ", gSL_Folga_ATR);

   double perda_estimada = 5.01 * LoteInicial / 0.1;
   double lucro_mensal   = 11.3 * LoteInicial / 0.1;

   Print("v31 BUY-ONLY | Lote:", LoteInicial,
         " | Perda media/trade estimada: ~", NormalizeDouble(perda_estimada,0), " EUR (",
         NormalizeDouble(perda_estimada/100.0,2), "% de 10K)",
         " | Meta mensal estimada: ~", NormalizeDouble(lucro_mensal,0), " EUR (",
         NormalizeDouble(lucro_mensal/100.0,2), "% de 10K)");
   Print("v31 | MinAfastATR:", gMinAfastamentoATR,
...
void OnTick()
{
   ...
   if(setupCompraAtivo && ask >= precoGatilho)
   {
      ...
      {
         trade.Buy(LoteInicial, _Symbol, ask, precoStopLoss, precoTakeProfit, "Falha de Fundo v31");
   ...
   if(setupVendaAtivo && bid <= precoGatilho)
   {
      ...
      if(maxGap <= 0 || overshoot <= maxGap)
      {
         trade.Sell(LoteInicial, _Symbol, bid, precoStopLoss, precoTakeProfit, "Falha de Topo v31");
