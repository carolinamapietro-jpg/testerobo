//+------------------------------------------------------------------+
//|                     EA Falha Exaustao v23.0                      |
//|  Remove filtro MA200 | ambas as direcoes ativas                   |
//+------------------------------------------------------------------+
//
//  CORRECAO LOGICA v23 (solicitada pelo usuario):
//
//  O filtro MA200 foi removido de ambas as direcoes (padrao: false).
//  Logica anterior exigia:
//    BUY:  MA20 > MA200 (tendencia de alta de longo prazo)
//    SELL: MA20 < MA200 (tendencia de baixa de longo prazo)
//
//  Por que foi removido:
//  A estrategia e de REVERSAO A MEDIA (MA20). O sinal e:
//    preco afastado da MA20 (2xATR) + padrao de falha de exaustao
//  A posicao da MA200 e irrelevante para essa reversao. O preco vai
//  reverter a MA20 ou nao, independente de onde esta a MA200.
//  BUY: preco 2xATR ABAIXO da MA20 => reverte para MA20 (sobe)
//  SELL: preco 2xATR ACIMA da MA20 => reverte para MA20 (cai)
//
//  UsarFiltroMA200 permanece como parametro para testes comparativos.
//
//  OUTRAS MUDANCAS v23:
//  [1] PermitirVenda = true (SELL reativado)
//  [2] Guard do MA200 condicional: nao bloqueia quando desligado
//  [3] Mantidos do v22: floors 2.0/0.8/1.5, anti-gap 0.5xATR
//
//  LOGICA DE PADRAO:
//  BUY:  preco 2xATR abaixo MA20 + L1<L2 && C1>=L2 (falha de fundo)
//  SELL: preco 2xATR acima MA20 + H1>H2 && C1<=H2 (falha de topo)
...
int OnInit()
{
   ...
   if(gMinAfastamentoATR != MinAfastamentoATR)
      Print("AVISO v23: MinAfastamentoATR ", MinAfastamentoATR,
            " elevado ao floor de ", gMinAfastamentoATR);
   if(gSL_Folga_ATR != SL_Folga_ATR)
      Print("AVISO v23: SL_Folga_ATR ", SL_Folga_ATR,
            " elevado ao floor de ", gSL_Folga_ATR);
   if(gBreakEvenATR != BreakEvenATR)
      Print("AVISO v23: BreakEvenATR ", BreakEvenATR,
            " elevado ao floor de ", gBreakEvenATR);

   Print("v23 parametros efetivos | MinAfastATR:", gMinAfastamentoATR,
...
void OnTick()
{
   ...
   if(setupCompraAtivo && ask >= precoGatilho)
   {
      ...
      {
         if(trade.Buy(LoteInicial, _Symbol, ask, precoStopLoss, precoTakeProfit, "Falha de Fundo v23"))
            tradesCompraHoje++;
   ...
   if(setupVendaAtivo && bid <= precoGatilho)
   {
      ...
      if(maxGap <= 0 || overshoot <= maxGap)
      {
         if(trade.Sell(LoteInicial, _Symbol, bid, precoStopLoss, precoTakeProfit, "Falha de Topo v23"))
            tradesVendaHoje++;
