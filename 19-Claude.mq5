//+------------------------------------------------------------------+
//|                     EA Falha Exaustao v19.0                      |
//|  Retorno ao pool de sinais do v13 | Eliminacao de filtros danosos |
//+------------------------------------------------------------------+
//
//  CAUSA RAIZ IDENTIFICADA (comparativo com dados reais):
//
//  v13 | sem ok_corpo | sem RSI | sem PavioCorpo | BUY win 54% | +91.58 EUR/ano
//  v18 | ok_corpo ON  | RSI ON  | PavioCorpo ON  | BUY win 34% | -132.47 EUR/ano
//
//  O filtro ok_corpo (C1>O1 para BUY / C1<O1 para SELL) adicionado em
//  v17 foi a causa da queda de 54% para 34% de acerto no BUY.
//  Ele bloqueava padroes validos: Falha de Fundo onde o candle fechou
//  acima de L2 (sinal real) mas abaixo do open (corpo bearish) e que
//  o v13 capturava com exito. O corpo NAO define a validade do padrao;
//  o que define e o fechamento ACIMA ou ABAIXO do nivel de referencia.
//
//  RSI (60/40) e PavioCorpo (1.5) foram removidos do codigo:
//  Em todos os testes historicos (v14 a v18) causaram reducao do win
//  rate quando ativos. Nao existe evidencia de melhora.
//
//  CORRECOES v19:
//  [1] Remocao do ok_corpo geral: padrao detectado como no v13
//      ok_padrao_venda  = sell_cond && (!buy_cond  || C1 < O1)
//      ok_padrao_compra = buy_cond  && (!sell_cond || C1 > O1)
//      CORPO so e usado para desambiguar candles que ativam AMBOS
//      os padroes simultaneamente (outside bars).
//  [2] Remocao total dos inputs RSI e PavioCorpo: nao ha parametro
//      para habilita-los; nao podem ser acidentalmente ativados.
//  [3] Mantidos do v18: SaidaDinamicaMinRR, BreakEvenATR=1.5,
//      MinBarrasHold=15, TP fixo 2:1, ATR thresholds, break-even.
//
//  LOGICA DE PADRAO (identica ao v13, sem filtro de corpo):
//  SELL: preco rompeu maxima anterior (H1>H2) mas fechou abaixo (C1<=H2)
//  BUY:  preco perdeu minima anterior (L1<L2) mas fechou acima (C1>=L2)
//  Desambiguacao por corpo aplicada apenas quando ambos sao verdadeiros.
//
//+------------------------------------------------------------------+
...
input double MinAfastamentoATR       = 1.5;   // Distancia minima da MA20 em ATR
input double PavioMinimoATR          = 0.25;  // Pavio minimo do candle de sinal
input double SL_Folga_ATR            = 1.0;   // Buffer SL alem do extremo do pavio
...
input double BreakEvenATR            = 1.5;   // Lucro (x ATR) para mover SL a entrada
...
void OnTick()
{
   ...
   if(barAtual != lastBar)
   {
      ...
      // Padroes primarios (identicos ao v13)
      ...
      // (outside bar com pavio sup e inf), usa o corpo para decidir direcao.
      // Quando apenas UM padrao ocorre, corpo NAO e exigido (como v13).
   ...
   {
      if(trade.Buy(LoteInicial, _Symbol, ask, precoStopLoss, precoTakeProfit, "Falha de Fundo v19"))
         tradesCompraHoje++;
      setupCompraAtivo = false;
   ...
   {
      if(trade.Sell(LoteInicial, _Symbol, bid, precoStopLoss, precoTakeProfit, "Falha de Topo v19"))
         tradesVendaHoje++;
      setupVendaAtivo = false;
