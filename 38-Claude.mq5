//+------------------------------------------------------------------+
//|                     EA Falha Exaustao v38.0                      |
//|  Filtro Daily MA50 para SELL | Filtro H4 | 3.5 lots | USDJPY M5  |
//+------------------------------------------------------------------+
//
//  DIAGNOSTICO v37 — SPIKE DE JULHO-AGOSTO 2024:
//
//  O relatorio 2023-2024 mostrou um salto de saldo de 5.253 → 11.463 EUR
//  entre agosto 2 e agosto 6 de 2024. Causa: intervencao do BOJ (Banco
//  do Japao) em 31/julho/2024 com alta inesperada de juros + reducao
//  de compra de bonds. Isso gerou o "Black Monday Japan" (5/ago/2024):
//    - USDJPY caiu de 161 → 141 em menos de 2 semanas
//    - Um dos maiores crashes do USDJPY em decadas (carry trade unwind)
//    - Os trades SELL capturaram essa queda: 2 trades sozinhos = +5.000 EUR
//    - Sem esse evento, o periodo 2023-2024 seria ~ -5.000 EUR
//
//  PROBLEMA IDENTIFICADO:
//  O SELL gerou 111 trades em 2025-2026 (bull market) com 28.83% win rate,
//  custando ~5.648 EUR. O filtro H4 nao e suficiente para bloquear SELL
//  durante correcoes temporarias num mercado estruturalmente de alta.
//
//  SOLUCAO v38 — FILTRO DAILY MA50 PARA SELL:
//  Novo handle MA50 no timeframe DIARIO.
//  SELL so opera quando: C1 (fechamento M5) < MA50(D1)[1]
//  Ou seja: o preco atual esta abaixo da MA50 Diaria
//    → mercado em bear market estrutural de verdade
//
//  POR QUE MA50 DIARIA:
//    - Em 2025-2026 (bull market): preco acima de MA50 D1 → SELL bloqueado ✓
//    - Em julho-agosto 2024 (crash BOJ): preco despencou abaixo MA50 D1
//      → SELL liberado → capturou os grandes lucros ✓
//    - Em 2023 (range/misto): SELL liberado apenas nos momentos genuinamente
//      bearish → qualidade dos sinais SELL muito maior
//
//  RESULTADO ESPERADO:
//    - 2025-2026: SELL praticamente bloqueado → BUY-only → deve voltar
//      ao nivel de v35 (+4.299 EUR)
//    - 2023-2024: apenas os SELL trades de alta qualidade permanecem
//      (crash BOJ incluido) → deve manter o resultado positivo
//
//+------------------------------------------------------------------+
...
input int    MA_Diaria_Period         = 50;    // MA50 no Daily — SELL so opera se
                                               // C1 < MA50(D1): preco em bear market real
...
int OnInit()
{
   ...
   if(gMinAfastamentoATR != MinAfastamentoATR)
      Print("AVISO v38: MinAfastamentoATR elevado ao floor de ", gMinAfastamentoATR);
   if(gSL_Folga_ATR != SL_Folga_ATR)
      Print("AVISO v38: SL_Folga_ATR elevado ao floor de ", gSL_Folga_ATR);

   Print("v38 | Lote:", LoteInicial,
   ...
   {
      Print("ERRO v38: falha ao criar handle de indicador.");
      return INIT_FAILED;
   }
...
void OnTick()
{
   ...
   if(barAtual != lastBar)
   {
      ...
      // --- FILTRO DAILY MA50 PARA SELL ---
      // SELL so opera quando preco esta abaixo da MA50 Diaria
      // Garante que so vendemos em bear markets estruturais reais
      // (bull market corrections bloqueadas: preco acima MA50 D1)
      ...
      // Filtro D1: C1 < MA50(D1) — preco em bear market estrutural real
      //------------------------------------------------------------
   ...
   if(setupCompraAtivo && ask >= precoGatilho)
   {
      ...
      {
         trade.Buy(LoteInicial, _Symbol, ask, precoStopLoss, precoTakeProfit, "Falha de Fundo v38");
   ...
   if(setupVendaAtivo && bid <= precoGatilho)
   {
      ...
      if(maxGap <= 0 || overshoot <= maxGap)
      {
         trade.Sell(LoteInicial, _Symbol, bid, precoStopLoss, precoTakeProfit, "Falha de Topo v38");
