//+------------------------------------------------------------------+
//|                     EA Falha Exaustao v24.0                      |
//|  Fix saida dinamica: MinRR=2.0 alinhado ao RiscoRetorno          |
//+------------------------------------------------------------------+
//
//  DIAGNOSTICO v23 (backtest 2025.01-2026.06, 769 trades):
//  - Taxa de acerto: 36.93% (acima do break-even 33.33% para RR=2.0) OK
//  - Media lucro/trade: 7.61 EUR  (deveria ser ~12.56 = 2x a perda)
//  - Media perda/trade: 6.28 EUR
//  - R:R real obtido:  1.21  (objetivo: 2.0)
//  - Fator de Lucro:   0.71  (negativo)
//
//  CAUSA RAIZ:
//  SaidaDinamicaMinRR=1.0 fechava posicoes quando lucro >= 1x risco.
//  Como o TP esta em 2x risco, a saida dinamica cortava os vencedores
//  exatamente no ponto medio entre entrada e TP. Resultado: o R:R real
//  foi 1.21 em vez de 2.0, destruindo a vantagem do sinal.
//
//  CORRECAO v24:
//  SaidaDinamicaMinRR = 2.0 (alinhado com RiscoRetorno)
//  Agora a saida dinamica so dispara se lucro >= 2x risco, ou seja,
//  o TP sempre bate primeiro => saida dinamica efetivamente inativa.
//  O sistema passa a depender puramente do TP/SL fixos.
//
//  MATEMATICA ESPERADA:
//  Taxa 37% x R:R 2.0 => Fator de Lucro = (0.37x2)/(0.63x1) = 1.17
//
//  MANTIDOS DO v23:
//  PermitirVenda=true, UsarFiltroMA200=false, floors 2.0/0.8/1.5,
//  anti-gap 0.5xATR, guard MA200 condicional
//
//+------------------------------------------------------------------+
...
int OnInit()
{
   ...
   if(gMinAfastamentoATR != MinAfastamentoATR)
      Print("AVISO v24: MinAfastamentoATR ", MinAfastamentoATR,
            " elevado ao floor de ", gMinAfastamentoATR);
   if(gSL_Folga_ATR != SL_Folga_ATR)
      Print("AVISO v24: SL_Folga_ATR ", SL_Folga_ATR,
            " elevado ao floor de ", gSL_Folga_ATR);
   if(gBreakEvenATR != BreakEvenATR)
      Print("AVISO v24: BreakEvenATR ", BreakEvenATR,
            " elevado ao floor de ", gBreakEvenATR);

   Print("v24 parametros efetivos | MinAfastATR:", gMinAfastamentoATR,
...
void OnTick()
{
   ...
   if(setupCompraAtivo && ask >= precoGatilho)
   {
      ...
      {
         if(trade.Buy(LoteInicial, _Symbol, ask, precoStopLoss, precoTakeProfit, "Falha de Fundo v24"))
            tradesCompraHoje++;
   ...
   if(setupVendaAtivo && bid <= precoGatilho)
   {
      ...
      if(maxGap <= 0 || overshoot <= maxGap)
      {
         if(trade.Sell(LoteInicial, _Symbol, bid, precoStopLoss, precoTakeProfit, "Falha de Topo v24"))
            tradesVendaHoje++;
